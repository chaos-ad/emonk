...
receive_response(State=#state{vm=VM, callback=Callback}) ->
    receive
        {emonk_response, Response} ->
            Response;
        {emonk_callback, Args} ->
            try
                case handle_callback(Callback, Args) of
                    {error, Result} -> emonk_nif:result(VM, {[{error, true}, {result, Result}]});
                    {ok, Result}    -> emonk_nif:result(VM, {[{error, false}, {result, Result}]});
                    Result          -> emonk_nif:result(VM, {[{error, false}, {result, Result}]})
                end
            catch _:Error ->
                emonk_nif:result(VM, {[{error, true}, {result, Error}]})
            end,
